...
const ParentMessages = ({ role, coachId }: { role: string, coachId: number | null }) => {
  ...
  useEffect(() => {
    if (selectedParticipant) {
      ...
      const interval = setInterval(() => fetchMessages(selectedParticipant.id), 5000);
  ...
  const fetchParticipants = async () => {
    try {
      ...
      const filtered = role === 'admin' ? data : data.filter((p: any) => p.coach_id === coachId);
      setParticipants(filtered);
      setLoading(false);
    } catch (e) {
      toast.error('Помилка завантаження учнів');
    }
  };
    ...
    try {
      ...
      const res = await fetch('/api/messages', {
        ...
        body: JSON.stringify({
          ...
          content: newMessage,
          sender_type: role === 'admin' ? 'admin' : 'coach'
        })
      });
      if (res.ok) {
        setNewMessage('');
    ...
    <div className="grid lg:grid-cols-4 gap-8 h-[calc(100vh-200px)]">
      <div className="lg:col-span-1 bg-zinc-900/50 rounded-[2.5rem] border border-white/5 flex flex-col overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-2 custom-scrollbar">
          {loading ? (
            <div className="p-8 text-center text-zinc-500">Завантаження...</div>
          ) : (
            participants.map(p => (
              <button
                key={p.id}
              ...
              >
                <div className={`w-10 h-10 rounded-xl flex items-center justify-center font-black ${
                  selectedParticipant?.id === p.id ? 'bg-white/20' : 'bg-red-600/10 text-red-600'
                }`}>
                  {p.name[0]}
                </div>
                <div className="text-left">
                  <p className="text-xs font-black uppercase tracking-tight truncate w-32">{p.name}</p>
                  <p className={`text-[8px] font-bold uppercase tracking-widest ${
                    selectedParticipant?.id === p.id ? 'text-white/60' : 'text-zinc-600'
                  }`}>
      ...
      <div className="lg:col-span-3 bg-zinc-900/50 rounded-[2.5rem] border border-white/5 flex flex-col overflow-hidden">
        {selectedParticipant ? (
          <>
            <div className="p-6 border-b border-white/5 bg-black/20 flex items-center justify-between">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h3 className="text-xl font-black uppercase tracking-tight">{selectedParticipant.name}</h3>
                  <p className="text-xs text-zinc-500 font-medium">Батьки: {selectedParticipant.parent_name || 'Не вказано'}</p>
                </div>
              </div>
            </div>
            ...
            <div className="flex-1 overflow-y-auto p-8 space-y-6 custom-scrollbar">
              {messages.map((msg, i) => (
                <div key={i} className={`flex ${msg.sender_type === 'parent' ? 'justify-start' : 'justify-end'}`}>
                  <div className={`max-w-[70%] p-5 rounded-3xl text-sm ${
                    ...
                    <div className={`text-[8px] mt-2 font-black uppercase tracking-[0.2em] ${
                      msg.sender_type === 'parent' ? 'text-zinc-500' : 'text-white/50'
                    }`}>
                      {new Date(msg.created_at).toLocaleTimeString('uk-UA', { hour: '2-digit', minute: '2-digit' })}
                    </div>
                  </div>
              ...
              {messages.length === 0 && (
                <div className="h-full flex flex-col items-center justify-center text-zinc-600 space-y-4">
                  ...
                  <p className="text-sm italic">Немає повідомлень. Напишіть першим!</p>
                </div>
              )}
            </div>
            ...
            <div className="p-6 bg-black/20 border-t border-white/5">
              <div className="flex gap-4">
                <input 
                  ...
                  placeholder="Напишіть повідомлення батькам..."
                  className="flex-1 bg-black border border-white/10 rounded-2xl px-6 py-4 text-sm outline-none focus:border-red-600/50 transition-all"
                />
                <button 
                  onClick={handleSendMessage}
                  className="bg-red-600 hover:bg-red-700 text-white px-8 rounded-2xl font-black uppercase tracking-widest text-xs transition-all shadow-lg shadow-red-600/20 active:scale-95"
                >
                  Надіслати
                </button>
              </div>
          ...
          <div className="h-full flex flex-col items-center justify-center text-zinc-600 space-y-6">
            ...
            <div className="text-center">
              <p className="text-lg font-black uppercase tracking-tight text-zinc-500">Оберіть чат</p>
              <p className="text-xs font-medium text-zinc-600 mt-2">Виберіть учня зі списку зліва, щоб розпочати спілкування</p>
            </div>
          </div>
        )}
      </div>
